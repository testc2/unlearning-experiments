...
class LR(nn.Module):
    ...
    def __init__(self,input_dim,sgd_seed=0):
        super(LR,self).__init__()
        self.fc1 = nn.Linear(input_dim,1,bias=False).double()
        torch.manual_seed(sgd_seed)
        weights = (torch.randn(input_dim)/math.sqrt(input_dim)).double()
        with torch.no_grad():
    ...
    class MyDataset(Dataset):
        def __init__(self, features, labels, ovr=False):
            
            self.data = features
            if ovr:
                self.labels = labels.double()
            else:
                self.labels = labels.view(-1).double()
        ...
        def __getitem__(self, index):
            ...
            return data.type(torch.DoubleTensor), target.type(torch.DoubleTensor), index
...
class Logistic_regression(nn.Module):
    ...
    def __init__(self, input_dim, output_dim):
        ...
        self.fc2 = nn.LogSoftmax(dim=1).double()

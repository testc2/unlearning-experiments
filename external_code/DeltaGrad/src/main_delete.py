...
def model_update_standard_lib(num_epochs, dataset_train, model, random_ids_multi_epochs, sorted_ids_multi_epochs, delta_data_ids, batch_size, learning_rate_all_epochs, criterion, optimizer, is_GPU, device, noise_vectors = None):
    count = 0
 ...
 #     
    ...
    for k in range(num_epochs):   
        ...
        for j in range(len(random_ids_list)):
            ...
            if k%100 ==0 and j==len(random_ids_list)-1:
                print(f"Epoch:{k} Batch: {j} Baseline Loss {loss.detach().cpu().item()}")
...
#     init_model_params = list(model.parameters())
    ...
    if args.remove_method == "Guo":
        noise_vectors = torch.load(git_ignore_folder + "noise_vectors")
#     hyper_params = [criterion, optimizer]
    ...
    if method == baseline_method:
        ...
        updated_model = model_update_standard_lib(num_epochs, dataset_train, model, random_ids_all_epochs, sorted_ids_multi_epochs, delta_data_ids, batch_size, learning_rate_all_epochs, criterion, optimizer, is_GPU, device ,noise_vectors)
...
#         torch.save(exp_grad_list, git_ignore_folder + 'exp_grad_list')    
        test(updated_model,dataset_test,batch_size,criterion,len(dataset_test),is_GPU,device)
        with torch.no_grad():
            compute_model_para_diff(list(origin_model.parameters()), list(updated_model.parameters()))
            test(updated_model,dataset_test,batch_size,criterion,len(dataset_test),is_GPU,device)
            print("Remove ",end="")
            test(updated_model,dataset_remove,batch_size,criterion,len(dataset_remove),is_GPU,device)
            print("Remain ",end="")
            test(updated_model,dataset_prime,batch_size,criterion,len(dataset_prime),is_GPU,device)
...
#             model_update_provenance_test3(period, 1, init_epochs, dataset_train, model, grad_list_all_epoch_tensor, para_list_all_epoch_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, max_epoch, 2, learning_rate_all_epochs, random_ids_multi_epochs, sorted_ids_multi_epochs, batch_size, dim, added_random_ids_multi_super_iteration, X_to_add, Y_to_add, criterion, optimizer, lr_scheduler, regularization_coeff, is_GPU, device)
            ...
            updated_model = model_update_deltagrad(num_epochs, period, 1, init_epochs, dataset_train, model, grad_list_all_epochs_tensor, para_list_all_epoch_tensor, grad_list_GPU_tensor, para_list_GPU_tensor, cached_size, delta_data_ids, m, learning_rate_all_epochs, random_ids_all_epochs, sorted_ids_multi_epochs, batch_size, dim, criterion, optimizer, regularization_coeff, is_GPU, device, noise_vectors)
...
#     sorted_random_ids_all_epochs = torch.load(git_ignore_folder + 'sorted_ids_multi_epochs_' + str(repetition))
    ...
    model, gradient_list_all_epochs, para_list_all_epochs, learning_rate_all_epochs = model_training_lr_test(random_ids_all_epochs, num_epochs, model, dataset_train, len(dataset_train), optimizer, criterion, batch_size, is_GPU, device, lr_lists, noise_vectors)
...
#     torch.save(random_ids_multi_epochs, git_ignore_folder + 'random_ids_multi_epochs')
    ...
    torch.save(noise_vectors, git_ignore_folder + 'noise_vectors')
